...
class GCN(nn.Module):
    def __init__(self, inputs, time_steps=90, pretrained= True , partialConv = False, fusion = 'avg'):
        ...
        if self.fusion == 'gcn':
            ...
            self.fc_emb_1 = nn.Linear(self.hidden_size, self.emb_size, bias=False)
    ...
    def message_passing(self, input_feature , trackers, dist_mask=None):
        ...
        if self.fusion == 'avg':
            ...
            ego_feature = emb_feature[:, 0, :].view(-1, 1, self.emb_size).repeat(1, num_box, 1)
            ...
            # ori_ego_feature : (B, H)
            ori_ego_feature = input_feature.view(-1, num_box, self.hidden_size)[:, 0, :]
            # input_feature : (BxN, H)
            input_feature = input_feature.view(-1, self.hidden_size)

            # fusion_feature : (B, N, H)
            fusion_feature = (input_feature * attn_weights).view(-1, num_box, self.hidden_size)
            # input_feature : (B, H)
    ...
    def forward(self, camera_inputs, trackers, device, dist_mask , mask = None):
        ...
        # box_ind : (BxN)
        box_ind = np.array([np.arange(batch_size)] * self.num_box).transpose(1, 0).reshape(-1)
        box_ind = torch.from_numpy(box_ind.astype(np.int32)).to(device)

        # initialize LSTM
        hx = torch.zeros((batch_size*self.num_box, self.hidden_size)).to(device)
        cx = torch.zeros((batch_size*self.num_box, self.hidden_size)).to(device)
        logit_vel_stack = []

        # CNN
        if self.partialConv:
            camera_inputs = self.backbone.features(camera_inputs.view(-1,c,w,h), mask.view(-1,c,w,h)) # camera_inputs :(bs,t,c,w,h)
        else:
            camera_inputs = self.backbone.features(camera_inputs.view(-1,c,w,h)) # camera_inputs :(bs,t,c,w,h)
        ...
        updated_feature = self.message_passing(hx , trackers, dist_mask) #BxH
